    ...
    public class DataGridViewNumericUpDownEditingControl : NumericUpDown, IDataGridViewEditingControl
    {
        ...
        public virtual bool EditingControlWantsInputKey(Keys keyData, bool dataGridViewWantsInputKey)
        {
            switch (keyData & Keys.KeyCode)
            {
                ...
                case Keys.Delete:
                    {
                        // Let the grid handle the key if the carret is at the end of the text./ 如果 carret 位于文本末尾，则让网格处理键。
                        TextBox textBox = this.Controls[1] as TextBox;
                        if (textBox != null)
                        {
        ...
        protected override void OnValueChanged(EventArgs e)
        {
            ...
            if (this.Focused)
            {
                // Let the DataGridView know about the value change等 DataGridView 值变化
                NotifyDataGridViewOfValueChange();
            }
        }
